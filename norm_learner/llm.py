...
1. MockLLM — returns a stub response; useful for unit tests or dry runs.
2. OpenAICompatibleLLM — calls any OpenAI-compatible REST endpoint using
   ...
   - OpenAI itself
3. LocalTransformersLLM — runs a local Qwen (or other causal-LM) model via
...
# ---------------------------------------------------------------------------
# Mock backend (testing / no-LLM baseline)
# ---------------------------------------------------------------------------

class MockLLM(LLMInterface):
    """Returns a deterministic stub response for testing."""

    def __init__(self, fixed_response: str | None = None) -> None:
        self._response = fixed_response or json.dumps([
            {
                "type": "unknown",
                "description": "Mock LLM: no real model attached.",
                "reasoning": "MockLLM returns this stub for testing purposes.",
            }
        ])

    def query(self, prompt: str) -> str:  # noqa: ARG002
        logger.debug("MockLLM.query called (returning stub).")
...
class OpenAICompatibleLLM(LLMInterface):
    ...
    model : str
        Model identifier, e.g. "qwen-plus", "qwen2.5-72b-instruct", etc.
...
class LocalTransformersLLM(LLMInterface):
    """
